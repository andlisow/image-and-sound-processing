...
public class QuadTree {
    ...
    public void process(int threshold){
        root.executeSplit(threshold);


        List<PixelPoint[][]> regions = getRegions();
        System.err.println(regions.size());
    }

    public List<PixelPoint[][]> getRegions(){
        List<PixelPoint[][]> regions = new ArrayList<>();
        QuadNode node = root;
        while(node.getFirstLeaf() != null && !node.visited){
            QuadNode newNode = node.getFirstLeaf();
            regions.add(newNode.values);
        }
        return regions;
